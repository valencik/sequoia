...
//    }}
  ...
  def resolveOneTablish[I](tablish: Tablish[RawName, I]): RState[Tablish[ResolvedName, I]] = State[Resolver, Tablish[ResolvedName, I]] { acc =>
    {
      tablish match {
        // TODO support alias
  ...
  def resolveFrom[I](from: From[RawName, I]): RState[From[ResolvedName, I]] = State[Resolver, From[ResolvedName, I]] { acc =>
    {
  ...
  def resolveExpression[I](exp: Expression[RawName, I]): RState[Expression[ResolvedName, I]] = State[Resolver, Expression[ResolvedName, I]] { acc =>
    exp match {
  ...
  def resolveSelection[I](selection: Selection[RawName, I]): RState[Selection[ResolvedName, I]] = State[Resolver, Selection[ResolvedName, I]] { acc =>
    selection match {
  ...
  def resolveSelect[I](s: Select[RawName, I]): RState[Select[ResolvedName, I]] = for {
  ...
  def resolveOneCTE[I](cte: CTE[RawName, I]): RState[CTE[ResolvedName, I]] = for {
  ...
  def resQueryWith[I](qw: QueryWith[RawName, I]): RState[Query[ResolvedName, I]] = for {
  ...
  def resolveQuery[I](q: Query[RawName, I]): RState[Query[ResolvedName, I]] = q match {
    case QuerySelect(i, qs) => resolveSelect(qs).map {s => QuerySelect(i, s)}
    case QueryLimit(i, limit, qs) => resolveSelect(qs).map {s => QueryLimit(i, limit, s)}
